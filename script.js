...
slider.forEach(slider => slider.addEventListener('input', editImage));

function editImage() {
    //мб Переписать массивом чтобы короче было 
    //let sliders = document.querySelectorAll(".slider");
    //let values = document.querySelectorAll(".show_value");
    ...
    let brightVal = document.querySelector('#brightVal');
    let blurVal = document.querySelector('#blurVal');
    let greyVal = document.querySelector('#greyVal');
    let hueVal = document.querySelector('#hueVal');
    let saturationVal = document.querySelector('#saturationVal');

    brightVal.textContent = bright.value;
    blurVal.textContent = blur.value;
    greyVal.textContent = grey.value;
    hueVal.textContent = hue.value;
    saturationVal.textContent = saturation.value;

    image.style.filter = 'grayscale(' + grey.value + '%) hue-rotate(' + hue.value + 'deg) brightness(' + bright.value + '%) blur(' + blur.value + 'px) saturate(' + saturation.value + ')';
    
    //context.filter = 'grayscale(' + grey.value + '%) hue-rotate(' + hue.value + 'deg) brightness(' + bright.value + '%) blur(' + blur.value + 'px) saturate(' + saturation.value + ')';
